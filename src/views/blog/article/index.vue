    ...
    <div class="container px-0 py-12 mx-auto">
      ...
      <div class="flex gap-8 justify-end title-font sm:text-2xl text-xl px-4">
        <div
          ...
          @click="btnFn.changeDisplayStatus(item.value)"
        >
          <svg-icon :icon-class="item.icon" class="cursor-pointer hover:shadow-lg" />
        </div>
      </div>
      <div v-if="displayStatus === 1" class="flex flex-wrap">
        <div v-for="item in list" :key="item.id" class="p-4 lg:w-1/3 sm:w-full">
          <div
            class="h-full bg-white bg-opacity-75 px-8 pt-16 pb-24 rounded-lg overflow-hidden text-center relative hover:shadow-lg transition-all duration-300 cursor-pointer"
          >
            ...
            <p class="leading-relaxed mb-3 line-clamp-3">{{ item.contentStr }}</p>
            <router-link :to="{ name: 'vArticleDetail', query: { id: item.id } }">
      ...
      <div v-else class="flex flex-col gap-4 p-4">
        <div
          v-for="item in list"
          :key="item.id"
          class="bg-white hover:shadow-lg rounded-lg transition-all duration-300 gap-4 cursor-pointer"
        >
          <div class="p-8 flex flex-wrap md:flex-nowrap">
            <div class="md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col">
              <span class="font-semibold title-font text-gray-700">{{ item.tag }}</span>
              <span class="mt-1 text-gray-500 text-sm">{{ item.updateTimeStr }}</span>
            </div>
            <div class="md:flex-grow">
              <h2 class="text-2xl font-medium text-gray-900 title-font mb-2">
                {{ item.title }}
              </h2>
              <p class="leading-relaxed line-clamp-3">{{ item.contentStr }}</p>
              <router-link :to="{ name: 'vArticleDetail', query: { id: item.id } }">
  ...
  export default defineComponent({
    setup() {
      ...
      const btnFn = {
        ...
        get: () => {
          state.list.length = 0;
          const dict = useBasicUse.dict;
          ArticleApi.page({}, state.pagination.current, state.pagination.pageSize)
            .then((res: any) => {
              res.list.forEach((e: any) => {
                e.tag = dict.toPCLabel[e.id];
                e.updateTimeStr = formatToDateTime(e.updateTime);
                e.url = e.bgPic.length ? `${MINIO}${e.bgPic[0]}` : null;
              });
